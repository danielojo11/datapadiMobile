...
const PrintPreview: React.FC<PrintPreviewModalProps> = ({ visible, onClose, batches }) => {
  ...
  console.log("batches", batches[0].pins)
  console.log("All Pins", allPins);

  const handlePrint = async () => {
    const pdf = await generatePDF({ pins: allPins, batches });
    console.log(pdf);
  };
        ...
        <View style={styles.modalContainer}>
          ...
          >
            ...
            <View style={styles.statsRow}>
              <View style={styles.statBox}>
                ...
                <Text style={styles.statValueDark}>{allPins.length}</Text>
              </View>
              <View style={styles.statBox}>
                <Text style={styles.statLabel}>TOTAL VALUE</Text>
                <Text style={styles.statValueBlue}>{CURRENCY}{totalValue.toLocaleString()}</Text>
              </View>
            </View>
            ...
            <View style={styles.previewContainer}>
              ...
              {allPins.length === 0 ? (
                <View style={{ padding: 40, alignItems: "center" }}>
                  ...
                  <Text style={{ marginTop: 12, color: "#6B7280", fontSize: 16, fontWeight: "500" }}>No PINs available to preview</Text>
                  <Text style={{ marginTop: 4, color: "#9CA3AF", fontSize: 13, textAlign: "center" }}>This batch might be empty or still processing.</Text>
                </View>
              ) : (
                <>
                      ...
                      <View key={i} style={styles.voucherCard}>
                        <View style={styles.voucherTop}>
                          ...
                          <View
                            style={[
                              styles.networkBadge,
                              pin.networkId === "AIRTEL" && { backgroundColor: "#333" },
                            ]}
                          >
                            <Text style={styles.networkText}>{pin.networkId}</Text>
                          </View>
                        </View>

                        <View style={styles.pinContainer}>
                          <Text style={styles.pinLabel}>VOUCHER PIN</Text>
                          <Text style={styles.pinText}>{pin.pinCode ? formatPin(pin.pinCode).substring(0, 14) : "No pins please"}...</Text>
                        </View>

                        <View style={styles.voucherBottom}>
                          <Text style={styles.metaText}>{CURRENCY}{pin.amount}</Text>
                          <Text style={styles.metaText}>{getUSSD(pin.networkId).substring(0, 5)}#</Text>
                        </View>
                      </View>
                    ))}
                  </View>
          ...
          <View style={styles.footer}>
            <TouchableOpacity
              style={[
                styles.printButton,
                allPins.length === 0 && { backgroundColor: "#9CA3AF" }
              ]}
              onPress={handlePrint}
              disabled={allPins.length === 0}
            >
              <Text style={styles.printButtonText}>
                ...
                />{" "}
                {allPins.length > 0 ? `Download ${allPins.length} Voucher(s)` : 'No Vouchers to Download'}
              </Text>
            </TouchableOpacity>
          </View>
        </View>
